...
void Scene_Menu::sDoAction(const Command& action)
{
	if (action.type() == "START")
	{
		...
		{
			if (m_menuIndex == 0) {
				m_game->changeScene("ARCADE", std::make_shared<Scene_Tussle>(m_game, m_levelPaths[m_menuIndex]));
			}
			if (m_menuIndex == 1) {
				m_game->changeScene("VERSUS", std::make_shared<Scene_Tussle>(m_game, m_levelPaths[m_menuIndex]));
			}
			if (m_menuIndex == 2) {
				m_game->changeScene("TUTORIAL", std::make_shared<Scene_Tussle>(m_game, m_levelPaths[m_menuIndex]));
			}
			if (m_menuIndex == 3) {
